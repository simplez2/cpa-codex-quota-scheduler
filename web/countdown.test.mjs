...
test('unstarted 5h exposes global admission countdown',()=>{const c=env({warmup_enabled:true,warmup_traffic:{hold_reason:'min_interval',next_allowed_at:new Date(Date.now()+60000).toISOString()}});const children=[];c.warmupWait({auth_id:'a',windows:[{window:'5h',cycle_started:false}]},{append:(...x)=>children.push(...x)});assert.equal(children.length,3);assert.match(children[2].textContent,/最早准入/);});
